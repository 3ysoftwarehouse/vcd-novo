...
def logo_directory_path(instance, filename):
    ...
    return 'imagens/escolas/logo_{0}_{1}{2}'.format(instance.nome_razao_social, instance.id, extension)
...
class Escola(Pessoa):
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            obj = Escola.objects.get(id=self.id)
...
# Deletar logo quando escola for deletada
...
class MinhaEscola(models.Model):
    m_escola = models.ForeignKey(
        Escola, on_delete=models.CASCADE, related_name='minha_escola', blank=True, null=True)
    m_usuario = models.ForeignKey(
        Usuario, on_delete=models.CASCADE, related_name='escola_usuario')